...
def download_image(url, our_dir):
    ...
    try:
        ...
        if res.status_code == 200:
            print("download image_dict successfully:{}".format(url))
            filename = os.path.join(our_dir, basename)
            with open(filename, "wb") as f:
                content = res.content
                # 使用Image解码为图片
                # image_dict = Image.open(BytesIO(content))
                # image_dict.show()
                # 使用opencv解码为图片
                content = np.asarray(bytearray(content), dtype="uint8")
                # image_dict = cv2.imdecode(content, cv2.IMREAD_COLOR)
                # cv2.imshow("Image", image_dict)
                # cv2.waitKey(1000)
                # f.write(content)
                # time.sleep(2)
            return filename
        ...
        print(e)
        return None
    print("download image_dict failed:{}".format(url))
    return None
...
def download_image_thread(url_list, our_dir, num_processes, remove_bad=False, Async=True):
    '''
    多线程下载图片
    :param url_list: image_dict url list

...
def stereo_calibrate(left_file, right_file, left_dir, left_prefix, right_dir, right_prefix, image_format, save_file,
                     square_size, width=9, height=6):
    """ Stereo calibration and rectification """
    objp, leftp, rightp = load_image_points(left_dir, left_prefix, right_dir, right_prefix, image_format, square_size,
                                            width, height)
...
if __name__ == '__main__':
    ...
    # --left_file left_cam.yml --right_file right_cam.yml --left_prefix left --right_prefix right --left_dir dataset --right_dir dataset --image_format png --square_size 0.020 --save_file stereo_cam.yml
    ...
    if args.width and args.height:
        stereo_calibrate(args.left_file, args.right_file, args.left_dir, args.left_prefix, args.right_dir,
                         args.right_prefix, args.image_format, args.save_file, args.square_size, args.width,
                         args.height)
    else:
        assert Exception("Error:chessboard width={},height={}".format(args.width, args.height))

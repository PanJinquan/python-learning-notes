...
def calibrate(dirpath, prefix, image_format, square_size, width=9, height=6,show=False):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--show',type=str2bool,nargs='?',const=True,help='Turn on or turn off flag')
    args = parser.parse_args()

    # Call the calibraton and save as file. RMS is the error rate, it is better if rms is less than 0.2
    ret, mtx, dist, rvecs, tvecs = calibrate(args.image_dir,
                                             args.prefix,
                                             args.image_format,
                                             args.square_size,
                                             args.width,

...
if __name__ == '__main__':
    ...
    # calibration_file = "data/stereo_cam.yml"
    # calibration_file = "stereo_cam.yml"
    ...
    while True:  # Loop until 'q' pressed or stream ends
        ...
        depth = stereo_depth_demo.get_depth(disparity=displ, Q=Q)
        depth = np.asarray(depth, dtype=np.uint16)
        depth_colormap = stereo_depth_demo.get_depth_colormap(depth, clipping_distance=1500)
        image_processing.addMouseCallback("depth_colormap", depth)
        image_processing.addMouseCallback("left", depth)
        image_processing.addMouseCallback("Disparity", depth)
        # Show the images

...
from tools import image_processing, file_processing
from core import geometry_3d_pose, rgbd_data


class StereoCamera():
    def __init__(self, invert_camera=False):
        cv2.namedWindow("depth")
        cv2.moveWindow("left_rectified", 0, 0)
        cv2.moveWindow("right_rectified", 640, 0)
        cv2.createTrackbar("num", "depth", 2, 10, lambda x: None)
        cv2.createTrackbar("blockSize", "depth", 5, 255, lambda x: None)
        model_path = "./modules/openpose/pb/graph_opt.pb"
        ...
        self.ecv.add_mouse_event("depth")
        if invert_camera:
    ...
    def start_capture(self):
        ...
        while True:
            ...
            # self.left_rectified, self.right_rectified = frameL, frameR

            # self.left_rectified = cv2.imread(
            #     '/media/dm/dm2/X2/binocularCamera/Project/DepthPose/modules/disparity/python-SAD/L.bmp',
            #     cv2.COLOR_GRAY2RGB)
            # self.right_rectified = cv2.imread(
            #     '/media/dm/dm2/X2/binocularCamera/Project/DepthPose/modules/disparity/python-SAD/R.bmp',
            #     cv2.COLOR_GRAY2RGB)
            # 获得视差图
            ...
            # self.g.show(self.left_rectified, self.depth, body_joint2D=None, body_rects=None)
            # self.g.show(self.left_rectified, self.depth, body_joint=None)
            self.display()
            #
            key = cv2.waitKey(1)
            if key == ord("q"):
    ...
    def display(self, ):
        ...
        # depth_colormap = cv2.applyColorMap(cv2.convertScaleAbs(self.depth, alpha=255 / config.clipping_distance),
        #                                    cv2.COLORMAP_JET)
        cv2.imshow("left_rectified", self.left_rectified)
        cv2.imshow("right_rectified", self.right_rectified)

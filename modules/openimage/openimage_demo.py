...
def convert2voc(data ,image_dir, dst_dir):
    for index in range(data.shape[0]):
        ...
        if not os.path.isfile(image_dir_list):
            continue
        # image = image_processing.read_image(os.path.join(image_dir, data.iloc[index,:]["ImageID"]+".jpg"))
...
if __name__ == '__main__':
    ...
    dst_dir = "../openimage/openimage_pro/train/image"
    ...
    #     print(image_path)
    #     print(index)
    # image_processing.show_image_rects("image", image, rect_list)
    # cv2.imshow("image", image)
    # cv2.waitKey(0)
    #     image = image_processing.read_image(image_path)
    #     rect_list = [[body.iloc[i,:]["x"]*image.shape[1] ,
    #                   body.iloc[i,:]["y"]*image.shape[0] ,
    #                   body.iloc[i,:]["width"]*image.shape[1],
    #                   body.iloc[i,:]["height"]*image.shape[0]]]
    #     image_processing.show_image_rects("image", image, rect_list)
    #     print(rect_list)